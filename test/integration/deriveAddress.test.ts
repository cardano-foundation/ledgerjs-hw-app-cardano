...
const address_hex_to_base58 = (addressHex: string) => utils.base58_encode(utils.hex_to_buf(addressHex as HexString))
    ...
    describe("Should successfully derive Shelley address", () => {
        for (const { testName, network, addressParams, result: expectedResult } of shelleyTestcases) {
            it(testName, async () => {
                ...
                expect(utils.bech32_encodeAddress(utils.hex_to_buf(addressHex as HexString))).to.equal(
                    expectedResult
                )
            })
        }
